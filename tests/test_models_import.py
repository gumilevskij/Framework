...
def test(rng= ["2025-1-1","2035-1-1"],freq=1):
    ...
    file_path = os.path.abspath(os.path.join(working_dir,'supplements/models/MPAF/model.yaml'))
    model_import(file_path=file_path,list_variables=['RS'],list_shocks=['SHK_RS'],shocks=[1],list_titles=['Policy Interest Rate'],freq=freq,rng=rng)
       
    # 2. Dynare model file
    # Unit shock to nominal interest rate
    file_path = os.path.abspath(os.path.join(working_dir,'supplements/models/MPAF/model.mod'))
    model_import(file_path=file_path,list_variables=['RS'],list_shocks=['SHK_RS'],shocks=[1],list_titles=['Policy Interest Rate'],freq=freq,rng=rng)
    ...
    file_path = os.path.abspath(os.path.join(working_dir,'supplements/models/TOY/RBC.model'))
    model_import(file_path=file_path,list_variables=['Y','C','K','r','A'],list_shocks=['ea'],shocks=[0.1],list_titles=['Macroeconomic Variables'],
